...
class Forward(nn.Module):
    def __init__(self, flags):
        ...
        self.cyl1 = lorNN(self.flags)
        self.cyl2 = lorNN(self.flags)
        self.cyl3 = lorNN(self.flags)
        self.cyl4 = lorNN(self.flags)
        self.lin_eps_inf = nn.Linear(layer_size, 1, bias=True)
        self.d = nn.Linear(layer_size, 1, bias=True)
    ...
    def forward(self, G):
        ...
        out1 = self.cyl1(out[:, 0::4])
        out2 = self.cyl2(out[:, 1::4])
        out3 = self.cyl3(out[:, 2::4])
        out4 = self.cyl4(out[:, 3::4])
        eps_inf = self.lin_eps_inf(F.relu(out))
        d = self.d(F.relu(out))

        eps = out1 + out2 + out3 + out4
        eps_inf = eps_inf.expand_as(eps).type(torch.cfloat)
        eps += eps_inf
